...
LRESULT SwitcherSettingsDialog::OnInitDialog(UINT, WPARAM, LPARAM, BOOL&) {
	...
	schema_list_.AddColumn(L"Schema Name", 0);
	WTL::CRect rc;
...
LRESULT SwitcherSettingsDialog::OnGetSchemata(WORD, WORD, HWND hWndCtl, BOOL&) {
	HKEY hKey;
	LSTATUS ret = RegOpenKey(HKEY_LOCAL_MACHINE, _T("Software\\Rime\\TypeDuck"), &hKey);
	if (ret == ERROR_SUCCESS) {
		WCHAR value[MAX_PATH];
		DWORD len = sizeof(value);
		DWORD type = 0;
		DWORD data = 0;
		ret = RegQueryValueExW(hKey, L"TypeDuckRoot", NULL, &type, (LPBYTE)value, &len);
...
LRESULT SwitcherSettingsDialog::OnOK(WORD, WORD code, HWND, BOOL&) {
	if (modified_ && settings_ && schema_list_.GetItemCount() != 0) {
		...
		if (count == 0) {
			MessageBox(_T("You must select at least one schema from the schema list."), _T("Empty Selection"), MB_OK | MB_ICONEXCLAMATION);
			delete selection;
			return 0;
		}
