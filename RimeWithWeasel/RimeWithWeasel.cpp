...
bool RimeWithWeaselHandler::_ShowMessage(Context& ctx, Status& status) {
  ...
  const bool is_chinese = isChinese();
  if (m_message_type == "deploy") {
    if (m_message_value == "start")
      tips = is_chinese ? L"重新整理中……" : L"Refreshing…";
    else if (m_message_value == "success")
      tips = is_chinese
                 ? L"重新整理完成。TypeDuck 現已可供使用。"
                 : L"Refresh completed. TypeDuck is now available for use.";
    else if (m_message_value == "failure")
      tips = is_chinese
                 ? L"重新整理時發生錯誤，請查看記錄檔 "
                   L"\n%TEMP%\\rime.TypeDuck\\rime.TypeDuck.*.INFO 以了解詳情。"
                 : L"An error occurred while refreshing. Please check the log "
                   L"files \n%TEMP%\\rime.TypeDuck\\rime.TypeDuck.*.INFO for "
                   L"more information.";
