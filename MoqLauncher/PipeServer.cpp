...
std::wstring PipeServer::getPipeName(const wchar_t *baseName) {
  ...
  if (GetUserNameW(username, &unlen)) {
    // add username to the pipe path so it will not clash with other users'
    // pipes.
    pipeName = L"\\\\.\\pipe\\";
    pipeName += username;
    pipeName += L"\\MoqiIM\\";
...
PipeClient *PipeServer::clientFromId(const std::string &clientId) {
  ...
  auto it = std::find_if(clients_.cbegin(), clients_.cend(),
                         [clientId](const PipeClient *client) {
...
void PipeServer::initPipe(uv_pipe_t *pipe, const wchar_t *appName,
...
void PipeServer::onNewClientConnected(uv_stream_t *server, int status) {
  ...
  auto client = new PipeClient{this, server_pipe->pipe_mode,
                               server_pipe->security_attributes};
  acceptClient(client);
  client->startRead();
}
...
int PipeServer::exec(LPSTR cmd) {
  ...
  RegisterApplicationRestart(L"/restarted",
                             0); // should not include executable name.
  ...
  if (::IsWindows8OrGreater()) {
    ...
    // for Windows >= 8 since older versions do not have app containers (metro
    // apps) in which connecting to pipes are blocked by default permission
    // settings.
  ...
  uv_listen(reinterpret_cast<uv_stream_t *>(&serverPipe_), 32,
  ...
  uv_thread_create(
      &uiThread,
      [](void *arg) { reinterpret_cast<PipeServer *>(arg)->runGuiThread(); },
      this);
...
LPCTSTR PipeServer::registerWndClass(WNDCLASSEX &wndClass) const {
  ...
  wndClass.lpfnWndProc = [](HWND hwnd, UINT msg, WPARAM wp,
                            LPARAM lp) -> LRESULT {
    if (msg == WM_NCCREATE) {
      ...
      auto pObj = reinterpret_cast<PipeServer *>(
...
void PipeServer::createShellNotifyIcon() {
  ...
  shellNotifyIconData_.uVersion =
      NOTIFYICON_VERSION_4; // newer version after Windows Vista
...
void PipeServer::showPopupMenu() const {
  // NOTE: according to the API doc of NOTIFYICONDATA, we should able to get x &
  // y pos from WPARAM. However, it does not work in Windows 10 during my
  // experiment. So we get cursor pos instead.
  POINT pos;
  ::GetCursorPos(&pos);
