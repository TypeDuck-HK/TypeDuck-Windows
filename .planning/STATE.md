...
- [Phase 06]: Phase 6 Plan 06: Release/nightly CI surfaces use TypeDuck-Windows, TypeDuck-Windows-backend, `${{ github.repository_owner }}/schema` aap2-alpha, a shared schema prune list, and typeduck-windows-ime artifact names. — Visible release surfaces must be TypeDuck-owned and schema packaging must use the TypeDuck-HK-owned schema source instead of rime-frost.
- [Phase 06]: Backend `buildMenu()` is retained only as stale/helper code; TypeDuck v1 `onMenu()` returns an empty unhandled payload so backend menu items are unreachable from the frontend.
