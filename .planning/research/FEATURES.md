...
| Dictionary/detail panel for selected candidate | Web alpha screenshots show a candidate list plus a dictionary panel; users will expect the same lookup experience. | HIGH | Build as a compact native panel tied to candidate selection. Parse the documented lookup-filter CSV payload into fields instead of rendering it as one opaque line. |
